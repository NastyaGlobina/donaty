...
$res="<ul style='display: flex;list-style-type: none; margin: 0 auto; width: 50%; justify-content: space-between; '>";
if($page>1) $res.=self::li(self::anchor("{$baseUri}/1","перша"));
...
if($page>1) $res.=self::li(self::anchor("{$baseUri}/".($page-1),$page-1));
        ...
        if($page<$count-1) $res.=self::li(self::anchor("{$baseUri}/".($page+2),$page+2));
         if($page<$count) $res.=self::li(self::anchor("{$baseUri}/".($count),"остання"));
$res.="</ul>";
return $res;
    }
}
